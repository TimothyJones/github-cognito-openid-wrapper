  ...
  describe('UserDetails endpoint', () => {
    ...
    describe('When the access token is good', () => {
      ...
      it('returns a sucessful body', () =>
        pact.executeTest((mockService) =>
          github(mockService.url)
    ...
    describe('When the access token is bad', () => {
      ...
      it('rejects the promise', () =>
        pact.executeTest((mockService) =>
          expect(
            github(mockService.url).getUserDetails('THIS_IS_MY_TOKEN')
  ...
  describe('UserEmails endpoint', () => {
    ...
    describe('When the access token is bad', () => {
      ...
      it('rejects the promise', () =>
        pact.executeTest((mockService) =>
          expect(
            github(mockService.url).getUserEmails('THIS_IS_MY_TOKEN')
          ).rejects.toThrow(new Error('Request failed with status code 400'))
        ));
    });
    describe('When there is a server error response', () => {
  ...
  describe('Authorization endpoint', () => {
    describe('always', () => {
      it('returns a redirect url', () => {
        pact.executeTest((mockService) => {
          expect(
            github(mockService.url).getAuthorizeUrl(
              'client_id',
              'scope',
              'state',
              'response_type'
            ...
            `${mockService.url}/login/oauth/authorize?client_id=client_id&scope=scope&state=state&response_type=response_type`
          );
        });
      });
    });
